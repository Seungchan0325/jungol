...
bool chk[MAXN];
int N, K, rin[MAXN], in[MAXN], par[MAXN], out[MAXN], t, w[MAXN];
vector<tuple<int, int, int>> v;
MaxLazySeg seg;

void dnc(int l, int r, int up, int p)
{
    if(r < l) return;
...
signed main()
{
    ...
    for(int i = 0; i < ssize(v); i++) {
        pq.update(in[i], out[i], w[i]);
    }

    int ans = 0;
    for(int i = 0; i < K; i++) {
        auto [wg, now] = pq.query(0, ssize(v)-1);
        ans += wg;
        now = rin[now];
        while(now != -1 && chk[now] == false) {
            chk[now] = true;
            pq.update(in[now], out[now], -w[now]);
            now = par[now];
        }
    }
    cout << ans << "\n";
